...
describe('when rendering table', () => {
  describe('given 2 columns', () => {
    ...
    it('should insert .json-viewer if cell value is a json string', () => {
      var html = renderer.renderCell(7, '{"legen": {"wait": {"forIt": "Dary"}}}');
      expect(html.is('td')).to.be(true);

      const jsonToggle = html.find('.json-switcher');
      const textToggle = html.find('.text-switcher');
      const jsonView = html.find('.json-view');
      const textView = html.find('.text-view');

      expect(jsonToggle.hasClass('active focus')).to.be(true);
      expect(textToggle.hasClass('active focus')).to.be(false);

      expect(jsonView.css('display')).to.not.equal('none');
      // text view should be hidden by default
      expect(textView.css('display')).to.equal('none');

      textToggle.click();
      expect(textView.css('display')).to.not.equal('none');
      expect(jsonView.css('display')).to.equal('none');
    });

    it('should insert .json-viewer if cell value is an escaped json string', () => {
      var html = renderer.renderCell(7, '{"legen": {"wait": {"forIt": "Dary"}}}');
      expect(html.is('td')).to.be(true);
      expect(html.find('.json-viewer')).to.not.be.undefined;
    });
